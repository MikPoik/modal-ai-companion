...
async def generate_avatar(
    prompt: str,credentials: str = Depends(authenticate)):
    try:
        avatar_url = await modal_agent.generate_avatar.remote(prompt)
...
async def prompt(input_data: Generation, token: str = Depends(authenticate)):
    ...
    def stream_generator():
        try:
            ...
            for token in modal_agent.run.remote_gen(input_data, agent_config):
                yield token
        except Exception as e:
            yield f"\ndata: Error: {str(e)}\n\n"
        #yield "\ndata: [DONE]\n\n"
