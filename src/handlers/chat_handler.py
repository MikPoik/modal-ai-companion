...
models_4k_context = [
    "lizpreciatior/lzlv_70b_fp16_hf", "Gryphe/MythoMax-L2-13b"
]
...
class ChatHandler:
    ...
    def prepare_messages(self, prompt: str,
                         agent_config: AgentConfig) -> List[Dict]:
        ...
        if (agent_config.character and agent_config.character.backstory
                and len(agent_config.character.backstory) > 1000):
        ...
        history = self.get_chat_history(
            agent_config)  # This returns List[Dict]
        messages.extend(
        ...
        return self.filter_messages_for_context(
            messages, max_context_size=max_context_size)

    def _format_system_prompt(self,
                              agent_config: AgentConfig,
                              updated_backstory=None) -> Optional[str]:
    ...
    def get_chat_history(self,
        ...
        history = self.file_service.load_json(
            agent_config.workspace_id,
            f'{agent_config.context_id}_chat.json') or []
        ...
        if full_history:
            valid_history = self.filter_messages_for_context(
                valid_history, agent_config.llm_config.context_size)
    ...
    def save_chat_history(self, messages: List[Dict],
        ...
        filepath = f"/bucket-mount/{agent_config.workspace_id}/{agent_config.context_id}_chat.json"
        self.file_service.save_json(messages, agent_config.workspace_id,
                                    filename)

    def append_chat_history(self, messages: List[Dict],
                            agent_config: AgentConfig):
        """
        ...
        existing_messages = self.file_service.load_json(
            agent_config.workspace_id, "default_chat.json") or []
        ...
        # Save messages
        self.file_service.save_json(existing_messages,
                                    agent_config.workspace_id,
                                    "default_chat.json")
    ...
    def filter_messages_for_context(
            self,
            messages: List[Dict],
            max_context_size: int = 4096) -> List[Dict]:
        """
        ...
        current_tokens = estimate_tokens(
            system_message) if system_message else 0
    ...
    def delete_chat_history(self, agent_config: AgentConfig) -> bool:
        ...
        return self.file_service.delete_file(agent_config.workspace_id,
                                             filename)
    ...
    def keep_last_image_message(self, messages: List[dict]) -> List[dict]:
        ...
        last_image_index = next((i for i in reversed(range(len(messages)))
                                 if messages[i].get('tag') == 'image'), None)
